...
export default function PayReservationPage({
  ...
  params: Promise<{ roomId: string; reservationId: string }>;
}) {
  ...
  const nights = useMemo(() => {
    ...
    const diffDays = Math.max(
      0,
      Math.round((e.getTime() - s.getTime()) / msDay)
    );
  ...
  const total = useMemo(() => {
    if (!room) return 0;
    return (
      Math.round((room.price_per_night * nights + Number.EPSILON) * 100) / 100
    );
  }, [room, nights]);
  ...
  return (
    <main className="p-8">
      <div className="flex items-center justify-between mb-6">
        ...
        <Link href={`/rooms/${roomId}`} className="text-sm text-blue-600">
          Back
        </Link>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid gap-4 max-w-md">
          ...
          <div className="rounded border p-4 bg-white/60">
            ...
            <p className="text-sm">
              Price per night: $
              {room?.price_per_night?.toFixed?.(2) || room?.price_per_night}
            </p>
          ...
          {String(reservation.status).toLowerCase() !== "active" && (
            <p className="text-xs text-gray-600">
              Esta reserva no está activa (posiblemente cancelada). No es
              posible realizar el pago.
            </p>
          )}
          {message && <p className="text-sm text-green-600">{message}</p>}
          {error && <p className="text-sm text-red-600">{error}</p>}
          {!message && !error && <p className="text-xs text-gray-600"></p>}
        </div>
      )}
    </main>
  );
